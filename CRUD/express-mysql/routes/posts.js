...
router.get('/', function (req, res, next) {
    //query
    connection.query('SELECT petugas,DATE_FORMAT(wktlogin, "%Y-%m-%d") as tanggal, jenisk, count(*) as jumlah_kendaraan, count(NULLIF(tarif,0)) as bayar, SUM(tarif) as total, SUM(tiketmasalah) AS minus, COUNT(NULLIF(tiketmasalah,0)) as masalah FROM keluar GROUP BY petugas, jenisk, Date(wktlogin) ORDER BY Date(wktlogin) asc', function (err, rows) {
        if (err) {
            req.flash('error', err);
            res.render('posts', {
                data: ''
